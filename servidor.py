...
import sqlite3, requests, json, webbrowser
...
def utilizadores(id = None):
    ...
    if request.method == "POST": #CREATE 
        ...
        if len(req) == 2: #CREATE UTILIZADOR <nome> <senha> 
            ...
            response = make_response("Utilizador criado com sucesso", 200)
            ...
            try:
                ...
                g.connection.commit()
                response = make_response("Música avaliada com sucesso", 200)
                response.headers["Content-Type"] = "application/json"
                return response
        ...
        if request.url == "http://localhost:5000/utilizadores": #READ ALL UTILIZADORES
        ...
        elif request.url == "http://localhost:5000/utilizadores/all/" + id: #READ ALL MUSICAS_U <id_user>
        ...
        if request.url == "http://localhost:5000/utilizadores": #DELETE ALL UTILIZADORES
            g.connection.execute("DELETE FROM utilizadores")
            response = make_response("Todos os dados dos utilizadores foram apagados", 200)
            response.headers["Content-Type"] = "application/json"
            return response

        elif request.url == "http://localhost:5000/utilizadores/all/" + id: #DELETE ALL MUSICAS_U <id_user>
...
def artistas(id = None):
    ...
    if request.method == "POST": #CREATE ARTISTA <id_spotify>
        try:
            r = requests.get("https://api.spotify.com/v1/artists/"+id, headers=header)
        ...
        response = make_response("Artista inserido na base de dados", 200)
        response.headers["Content-Type"] = "application/json"
        return response

    elif request.method == "GET": #READ

        if request.url == "http://localhost:5000/artistas": #READ ALL ARTISTAS 
        ...
        elif request.url == "http://localhost:5000/artistas/all/" + id: #READ ALL MUSICAS_A <id_artista>
            ...
            try:
                ...
                r = requests.get("https://api.spotify.com/v1/artists/"+id_spotify,headers=header)
        ...
        if request.url == "http://localhost:5000/artistas": #DELETE ALL ARTISTA
            g.connection.execute("DELETE FROM artistas")
            response = make_response("Todos os dados dos artistas foram apagados", 200)
            response.headers["Content-Type"] = "application/json"
            return response

        elif request.url == "http://localhost:5000/artistas/all/" + id: #DELETE ALL MUSICAS_A <id_artista>
...
def musicas(id = None):
    ...
    if request.method == "POST": #CREATE MUSICA <id_spotify>
        try:
            r = requests.get("https://api.spotify.com/v1/tracks/" + id, headers=header)
            data = json.loads(r.content.decode()) #obtém os dados em json e transforma num dicionário
        ...
        if len(dados_artista) == 0: #caso não obtenha nenhuma linha
            r = requests.post('http://localhost:5000/artistas/' + id_spotify_artista)
            cursor = g.connection.execute("SELECT id FROM artistas WHERE id_spotify=(?)",(id_spotify_artista,))
            dados_artista = cursor.fetchall()

        g.connection.execute("INSERT INTO musicas (id_spotify,nome,id_artista) VALUES (?,?,?)",(id,nome_musica,dados_artista[0][0]))
        response = make_response("Música inserida na base de dados", 200)
        response.headers["Content-Type"] = "application/json"
        return response
    
    elif request.method == "GET": #READ

        if request.url == "http://localhost:5000/musicas": #READ ALL MUSICAS
        ...
        elif request.url == "http://localhost:5000/musicas/all/"+id: #READ ALL MUSICAS <avaliacao>
            ...
            try:
                ...
                registo = cursor.fetchone()
                id_spotify = registo[1]
                r = requests.get("https://api.spotify.com/v1/tracks/" + id_spotify, headers=header)
                data = json.loads(r.content.decode())
                if "error" in list(data.keys()): #caso o token esteja expirado
                    response = make_response("TOKEN expirado", 400)
                    response.headers["Content-Type"] = "application/json"
        ...
        if request.url == "http://localhost:5000/musicas": #DELETE ALL MUSICAS
            g.connection.execute("DELETE FROM musicas")
            response = make_response("Todos os dados das músicas foram apagados", 200)
            response.headers["Content-Type"] = "application/json"
            return response

        elif request.url == "http://localhost:5000/musicas/all/" + id: #DELETE ALL MUSICAS <avaliacao>
            cursor = g.connection.execute("SELECT id FROM avaliacoes where sigla=(?)", (id,))
            content_avalicao = cursor.fetchone()
            if content_avalicao == None:
                response = make_response("Tipo de avaliação não é válida", 400)
...
if __name__ == '__main__':
    app.run(debug = True) #qualquer alteração que fazemos ele sabe
